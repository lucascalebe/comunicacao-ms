...
class OrderService {
    async createOrder(req) {
        try {
            ...
            let order = this.createInitalOrderData(orderData, authUser, transactionid, serviceid);
    ...
    createInitalOrderData(orderData, authUser, transactionid, serviceid) {
        return {
            status: PENDING,
            user: authUser,
            createdAt: new Date(),
    ...
    async findAll() {
        try {
            ...
            console.info(`Response to GET all orders | [transactionID: ${transactionid} | serviceID: ${serviceid}]`);
            return response;
        } catch (err) {
            return {
                status: err.status ? err.status : INTERNAL_SERVER_ERROR,
